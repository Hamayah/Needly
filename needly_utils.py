...
def reset_needly_display_order(user_id: int):
    # Fetch all entries for the user ordered by created_at
    user_entries = Needly.select().where(
        Needly.user == user_id).order_by(Needly.created_at)
    # Assign display_id sequentially
    with db.atomic():
        for idx, record in enumerate(user_entries, start=1):
...
def log_entry(chat_id, user_id, log_cat, log_amt, log_desc, date):
    ...
    user_id = needly_entry.user.id
    # Reset the display order after logging an entry
    reset_needly_display_order(user_id)

    return needly_entry


def delete_entry_db(del_id: int, user_id: int) -> tuple:
    # Correct user_id reference from the User table
    user = User.get(User.user_id == user_id)
    needly_entry = Needly.get_or_none(
        (Needly.display_id == del_id) & (Needly.user == user.id))

    if needly_entry:
        delete = Needly.delete().where((Needly.display_id == del_id)
                                       & (Needly.user == user.id)).execute()
        # Reset the display order for the specific user after deleting a record
        reset_needly_display_order(user.id)
        return delete
    else:
        return 0
